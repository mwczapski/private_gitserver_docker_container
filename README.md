...
The scripts source a number of utility sdcripts, located in its `utils` subdirectory
...
In a suitable location in the file system, create a directory to host the package. You might consider <strong>gitserver</strong>, since this package will create an image suitable for running a git servere with latest git, and a container based on that image ready for use.
...
The script accepts no command line arguments and must expects the image produces by `01_create_gitserver_image.sh` to be available.
